...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	 ...
	 * 在任何的Bean属性后调用，创建次Servlet的WebApplicationContext
	 */
	@Override
	 ...
	 * 初始化并发布Servlet的WebApplicationContext，用于创建上下文，可以在子类中实现
	 *
	...
	protected WebApplicationContext initWebApplicationContext() {
		// 先从ServletContext中获得到父容器WebApplicationContext
		...
		// 先从ServletContext中查找Web容器的引用是否存在，并且创建好默认的空的IOC容器
		if (wac == null) {
			// No context instance was injected at construction time -> see if one
			// has been registered in the servlet context. If one exists, it is assumed
			// that the parent context (if any) has already been set and that the
			// user has performed any initialization such as setting the context id
			//在构造时没有注入上下文实例->查看是否有一个,已在servlet上下文中注册。 如果存在，则假定,已经设置了父上下文（如果有），并且用户已执行任何初始化，例如设置上下文ID
		...
		if (!this.refreshEventReceived) {
			...
			//上下文不是带有刷新的ConfigurableApplicationContext支持或在构建时注入的上下文刷新->在此处手动触发初始onRefresh
	 ...
	 * 从ServletContext中检索WebApplicationContext，属性和setContextAttribute配置名称的WeApplicationContext
	 * 必须已经加载并且存储在，初始化类中
	 *
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac) {
		...
		//在上下文中无论如何都将调用wac环境的#initPropertySources刷新； 请在此处急切地执行此操作，以确保servlet属性源到位用于以下#refresh之前的任何后处理或初始化
