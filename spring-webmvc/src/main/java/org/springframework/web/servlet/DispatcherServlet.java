...
public class DispatcherServlet extends FrameworkServlet {
	 ...
	 * 初始化此servlet使用的策略对象，可以在子类中重写，以初始化更多的策略对象。
	 */
	protected void initStrategies(ApplicationContext context) {
		...
		// 初始化此类使用的LocaleResolver,如果在BeanFactory中没有为此名称空间定义给定名称的bean，则默认为AcceptHeaderLocaleResolver。
		// 本地语言环境
		initLocaleResolver(context);

		// 初始化此类使用的ThemeResolver。如果在BeanFactory中没有为此名称空间定义给定名称的bean，则默认为FixedThemeResolver
		// 初始化模板处理器
		initThemeResolver(context);

		// 初始化此类使用的HandlerMappings。 如果在BeanFactory中没有为此名称空间定义HandlerMapping Bean，则默认使用BeanNameUrlHandlerMapping。
		// 初始化HandlerMappings
		initHandlerMappings(context);

		// 初始化此类使用的HandlerAdapters。如果在BeanFactory中没有为此名称空间定义HandlerAdapter Bean，则默认为SimpleControllerHandlerAdapter。
		// 初始化参数适配器
		initHandlerAdapters(context);

		// 初始化此类使用的HandlerExceptionResolver。如果在BeanFactory中没有为此名称空间定义给定名称的bean，则默认不使用任何异常解析器。
		// 初始化异常拦截器
		...
		// 初始化此类使用的ViewResolver。如果在BeanFactory中没有为此名称空间定义ViewResolver bean，则默认为InternalResourceViewResolver。
		// 初始化视图转换器
		initViewResolvers(context);

		// 初始化此Servlet实例使用的{@link FlashMapManager}。如果未配置任何实现，则默认为{@code org.springframework.web.servlet.support.DefaultFlashMapManager}
		// 初始化Flashmap管理器
	 ...
	 * 如果在BeanFactory中没有为此名称空间定义给定名称的bean，则默认为AcceptHeaderLocaleResolver。
	 */
