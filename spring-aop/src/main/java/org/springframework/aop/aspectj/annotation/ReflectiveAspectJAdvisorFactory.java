...
public class ReflectiveAspectJAdvisorFactory extends AbstractAspectJAdvisorFactory implements Serializable {
	...
	public List<Advisor> getAdvisors(MetadataAwareAspectInstanceFactory aspectInstanceFactory) {
		...
		// 我们需要用装饰器包装MetadataAwareAspectInstanceFactory 这样它只会实例化一次。
		...
		if (!advisors.isEmpty() && lazySingletonAspectInstanceFactory.getAspectMetadata().isLazilyInstantiated()) {
			// 如果寻找的增强器不为空而且有配置了增强延迟初始化，name需要在收尾加入同步实例化增强器
	...
	private AspectJExpressionPointcut getPointcut(Method candidateAdviceMethod, Class<?> candidateAspectClass) {
		...
		// 使用AspectJExpressionPointcut实例封装获取到的信息
		AspectJExpressionPointcut ajexp =
				new AspectJExpressionPointcut(candidateAspectClass, new String[0], new Class<?>[0]);
		// 提取注解中的表达式并且设置到ajexp实例对象中
		// 例如@Pointcut("execution(* *.*test*(..))")中的execution(* *.*test*(..))
	...
	public Advice getAdvice(Method candidateAdviceMethod, AspectJExpressionPointcut expressionPointcut,
		...
		//如果到达这里，就知道我们有一个AspectJ方法。检查它是一个由AspectJ注释的类
