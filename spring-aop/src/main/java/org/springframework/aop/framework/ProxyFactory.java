...
public class ProxyFactory extends ProxyCreatorSupport {
	 ...
	 * @return the proxy object根据此工厂中的设置创建一个新的代理。
	 * 可以反复调用。 如果我们添加或删除了界面，效果会有所不同。 可以添加和删除拦截器。使用给定的类加载器（如果需要创建代理）。 @param classLoader用于使用以下命令创建代理的类加载器（或{@code null}为低级代理工具的默认值）@返回代理对象
	 */
