...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	 ...
	 * 默认bean的作用于（Singleton)单例状态，可以在父类中进行覆盖
	 */
	...
	// 该属性设置为false这样容器在查找自动装配对象时不考虑该bean，即该bean不会被考虑作为其他bean自动装配的候选者，但是该bean本身还是可以使用自动装配来注入其他的bean
	private boolean autowireCandidate = true;

	// 出现多个自动装配的bean的候选者时，有该属性的将作为首选者
	...
	// 对应bean的factory-bean
	// <bean id="testBean" class="com.csh.TestBean"></bean>
	// <bean id="currentTime" factory-bean="testBean" factory-method="createTiem"></bean>
	@Nullable
	...
	// 方法重写持有者，记录lookup-method replace-method元素
	 ...
	 * 验证并准备给定的方法重写。检查是否存在具有指定名称的方法，如果未找到，则将其标记为未过载。
	 *
