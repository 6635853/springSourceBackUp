...
public abstract class AbstractBeanDefinitionReader implements BeanDefinitionReader, EnvironmentCapable {
	 ...
	 * 该方法主要完成两件事情
	 * 1、首先调用资源加载器的获取资源方法resourceLoader.getResource(location)方法，获取到指定的资源
	 * 2、真正执行加载功能，尤其子类XMLBeanDefinitionReader的loadBeanDefinitions()实现
	 *
	...
	public int loadBeanDefinitions(String location, @Nullable Set<Resource> actualResources) throws BeanDefinitionStoreException {
		...
		if (resourceLoader instanceof ResourcePatternResolver) {
			...
			try {
				// 将指定的bean的配置信息解析为SpringIOC容器封装的资源，加载多个指定位置的Bean的配置信息
				Resource[] resources = ((ResourcePatternResolver) resourceLoader).getResources(location);
				// 委派调用其子类的loadBeanDefinitions去实现
