...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	 ...
	 * if none specified.
	 * 通过项目本身提供一个如何寻找DTD声明的方法，有程序来实现寻找DTD声明的过程
	 * 默认是通过网络来下载获取，但是如果网络阻塞等无法下载，则会报错
	 */
	...
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		// 1、封装EncodedResource对象，主要是对资源文件的编码进行处理
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		// 从已封装的EncodedResource中获取Resource对象，并在此获取InputStream对象
		try (InputStream inputStream = encodedResource.getResource().getInputStream()) {
			// 2、根据获取到的InputStream构造InputSource实例
			InputSource inputSource = new InputSource(inputStream);
			if (encodedResource.getEncoding() != null) {
				inputSource.setEncoding(encodedResource.getEncoding());
			}
			// 3、根据Resource实例，和InputSource实例调用真正的处理解析文件的逻辑方法
	 ...
	 * 该方法中主要有三件事情
	 * 1、获取XML文件验证模式，在doLoadDocument（）方法中
	 * 2、加载XML文件，获取到对应的Document文档对象
	 * 3、根据Document对象，注册并返回Bean的信息
	 */
	 ...
	 * 获取配置文件的验证模式，该验证模式可以在XMLBeanDefinitionReader中通过setValidationMode方法进行设置
	 */
	protected int getValidationModeForResource(Resource resource) {
		...
		// 如果没有指定，则使用自动的验证模式，自动检测将其封装到XmlValidationMOdeDetector进行验证
	...
	protected int detectValidationMode(Resource resource) {
		...
		try {
			// 到XMLValidationModeDetector进行验证
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		// 实例化BeanDefinitioinDocumentReader对象
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();
		// 首先通过getRegistry()获取到BeanDefinitionRegistry对象，之后根据BeanDefinitionRegistry的实现类
		// DefaultListableBeanFactory获取到beanDefinitionMap的size，
		// 该个数是记录之前加载的BeanDefinition的个数（个人理解是Spring内部提供的一些默认BeanDefinition）
		int countBefore = getRegistry().getBeanDefinitionCount();
		// 根据文档对象获取到BeanDefinition
